        ...
        const fetchTerms = async () => {
            try {
                ...
                const response = await fetch(
                    `https://api.klubit.io/v1/legal-notices?language=${apiLanguage}`
                );

                if (!response.ok) {
                    throw new Error('Error al cargar los avisos legales');
                }

                const apiData: ApiResponse = await response.json();

                // Extraer las secciones del objeto anidado
                if (apiData.data && apiData.data.legalNotice) {
                    setSections(apiData.data.legalNotice.sections);
                } else {
