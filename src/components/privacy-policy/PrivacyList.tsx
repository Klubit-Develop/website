        ...
        const fetchTerms = async () => {
            try {
                ...
                const response = await fetch(
                    `https://api.klubit.io/v1/privacy-policy?language=${apiLanguage}`
                );

                if (!response.ok) {
                    throw new Error('Error al cargar la política de privacidad');
                }

                const apiData: ApiResponse = await response.json();

                // Extraer las secciones del objeto anidado
                if (apiData.data && apiData.data.privacyPolicy) {
                    setSections(apiData.data.privacyPolicy.sections);
                } else {
